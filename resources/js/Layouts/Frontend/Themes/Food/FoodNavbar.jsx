            ...
            <div className="sticky top-0 z-50 bg-white shadow-sm">
                <header className="border-b border-gray-100">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        {/* Reduced vertical padding from py-4 to py-2 on mobile */}
                        <div className="flex items-center justify-between py-2 lg:py-4 gap-4">
                            ...
                            {/* Logo - Reduced height on mobile (h-8) */}
                                ...
                                <Link href={route('home')} className="flex-shrink-0">
                                    {!logoError ? (
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[#f97316] rounded-xl sm:rounded-2xl flex items-center justify-center">
                                                <span className="text-white text-xl sm:text-3xl">🌱</span>
                                            </div>
                                            <div className="leading-none hidden sm:block">
                                                <span className="text-[#f97316] font-bold text-[18px] sm:text-[22px] tracking-tighter">GHORER</span>
                                                <span className="block text-[#01201d] font-bold text-[18px] sm:text-[22px] tracking-tighter -mt-1">BAZAR</span>
                                            </div>
                                        </div>
                                    )}
                                </Link>
                            ...
                            {/* Right Actions */}
                            <div className="flex-none lg:flex-1 flex items-center justify-end gap-2 sm:gap-4 lg:gap-6">
                                {/* Desktop Actions */}
                                ...
                                <button
                                    ...
                                    className="md:hidden p-2 text-gray-700 hover:text-[#f97316]"
                                >
                                ...
                                >
                                    ...
                                    {liveCartCount > 0 && (
                                        <span className="absolute -top-1 -right-1 bg-[#f97316] text-white text-[10px] font-bold rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center">
                                            {liveCartCount > 99 ? '99' : liveCartCount}
                                        </span>
                                    )}
                                </button>
                ...
                {showMobileSearch && (
                    <div className="md:hidden border-b border-gray-100 bg-white px-4 py-3 shadow-inner">
                        <form onSubmit={handleSearch} className="relative w-full">
                            <input
                                ...
                                placeholder="Search in..."
                                autoFocus
                                className="w-full bg-[#f8f9fa] border border-gray-200 focus:border-[#f97316] rounded-full py-2.5 pl-5 pr-12 text-sm placeholder-gray-500 focus:outline-none"
                            />
                            <button
                                type="submit"
                                className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#f97316]"
                ...
                <div className="bg-[#01201d] text-white hidden lg:block border-t border-gray-800">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <nav 
                            className="flex items-center gap-8 py-3.5 text-sm font-medium 
                            ...
                            {featuredCategories.length > 0 ? (
                                featuredCategories.slice(0, 10).map((cat, i) => (
                                    <Link
                                        ...
                                        className="hover:text-[#f97316] transition-colors flex items-center gap-1.5 flex-shrink-0"
                                    >
                                        {cat.name}
