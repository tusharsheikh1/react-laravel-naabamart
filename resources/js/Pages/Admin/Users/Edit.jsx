...
export default function Edit({ user, availablePermissions = [] }) {
    const { data, setData, put, processing, errors } = useForm({
        name: user.name || '',
        email: user.email || '',
        role: user.role || 'user',
        is_active: user.is_active ?? true, // Added for active status (defaults to true if null)
        password: '',
        password_confirmation: '',
        permissions: user.permissions || [], // Initialize with existing permissions
    });
            ...
            <div className="py-12">
                <div className="max-w-2xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white shadow-sm sm:rounded-lg p-6">
                        <h2 className="text-2xl font-semibold mb-6">Edit User: {user.name}</h2>
