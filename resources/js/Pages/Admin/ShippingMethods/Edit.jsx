...
export default function Edit({ shippingMethod }) {
    // 1. Switched `put` to `post`
    // 2. Added `_method: 'PUT'` for Laravel Method Spoofing
    // 3. Added safe fallbacks so React strictly controls the inputs
    const { data, setData, post, processing, errors } = useForm({
        name: shippingMethod?.name || '',
        base_charge: shippingMethod?.base_charge ?? 0,
        base_weight: shippingMethod?.base_weight ?? 1,
        additional_charge_per_kg: shippingMethod?.additional_charge_per_kg ?? 0,
        free_delivery_threshold: shippingMethod?.free_delivery_threshold ?? '',
        status: shippingMethod?.status === 1 || shippingMethod?.status === true,
        _method: 'PUT', 
    });

    const submit = (e) => {
        e.preventDefault();
        // Use post() to submit the spoofed PUT request
        post(route('admin.shipping-methods.update', shippingMethod.id), {
            preserveScroll: true,
        });
    };
        ...
        <Layout>
            ...
            <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div>
                        <InputLabel htmlFor="name" value="Zone/Method Name" />
                        <TextInput 
                            id="name" 
                            type="text" 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <TextInput 
                                ...
                                step="0.01" 
                                className="mt-1 block w-full" 
                                value={data.additional_charge_per_kg} 
                                onChange={(e) => setData('additional_charge_per_kg', e.target.value)} 
                            />
                    ...
                    <div className="flex items-center justify-end mt-4">
                        <PrimaryButton className="ml-4" disabled={processing}>Update</PrimaryButton>
                    </div>
                </form>
            </div>
        </Layout>
